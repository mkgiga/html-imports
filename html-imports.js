...
(() => {
  ...
  // run this script only once, at the beginning of the document
  ...
  log().col("cyan").fs("2em").txt("[ html-imports ]").send();
      ...
      if (lastParsedInvalidSyntaxType == null) {
        ...
        if (child.nodeType === Node.COMMENT_NODE) {
          ...
          if (nodeName.startsWith("@")) {
            currentSpecialSyntaxBlock["closing"] =
              "</on-" + nodeName.slice(1) + ">";
          }
    ...
    let component = class HTMLComponent extends HTMLElement {
      ...
      attributeChangedCallback(name, oldValue, newValue) {
        if (_attribute) {
          ...
          new Function("name", "oldValue", "newValue", _attribute).call(
            this,
            name,
            oldValue,
            newValue
